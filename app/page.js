...
`;
  ...
  return (
    <div className="container">
              ...
              {filtrados.filter(p=>p.categoria===cat && p.estado===est).map(p => {
                ...
                return (
                  <div className="card" key={p.id}>
                    ...
                    {(p.proximosPassos||[]).length>0 && <div className="muted"><strong>Próximos passos:</strong> {p.proximosPassos.join(" · ")}</div>}
                    ...
                    {/* NOVO: últimas entradas do histórico logo no cartão */}
                    {ult3.length>0 && (
                      <div className="timeline">
                        <div className="muted"><strong>Histórico recente:</strong></div>
                        {ult3.map(h=>(
                          <div key={h.id} className="flex">
                            <span className="badge">{h.origem}</span>
                            <span>{new Date(h.data).toLocaleString()}</span>
                            <span className="muted" style={{whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>{h.resumo || "(sem resumo)"}</span>
                          </div>
                        ))}
                      </div>
                    )}
      ...
      {/* NOVO: Reunião/Email modal */}
      {showHist && (
        <div className="modalBg" onClick={(e)=>{ if(e.target===e.currentTarget) setShowHist(false); }}>
          <div className="modal">
            <h3>Nova entrada: Reunião / Email / Nota</h3>
            <div className="grid">
              <select className="select" value={histTarget} onChange={e=>setHistTarget(e.target.value)}>
                <option value="">Escolhe o projeto</option>
                {projetos.map(p=> <option key={p.id} value={p.id}>{p.titulo}</option>)}
              </select>
              <div className="row2">
                <select className="select" value={histTipo} onChange={e=>setHistTipo(e.target.value)}>
                  <option value="reunião">Reunião</option>
                  <option value="email">Email</option>
                  <option value="nota">Nota</option>
                </select>
                <input className="input" type="date" value={histData} onChange={e=>setHistData(e.target.value)} />
              </div>
              <input className="input" placeholder="Resumo curto (1–2 linhas)" value={histResumo} onChange={e=>setHistResumo(e.target.value)} />
              <textarea className="textarea" rows={8} placeholder="Conteúdo completo (cola aqui a ata ou email)" value={histConteudo} onChange={e=>setHistConteudo(e.target.value)} />
              <label className="flex"><input type="checkbox" checked={histAutoIntegrar} onChange={e=>setHistAutoIntegrar(e.target.checked)} /> Integrar automaticamente em próximos passos/timeline/tags/responsáveis</label>
              <div className="flex" style={{justifyContent:"flex-end"}}>
                <button className="btn" onClick={()=>setShowHist(false)}>Cancelar</button>
                <button className="btn primary" onClick={guardarHistorico}>Guardar</button>
              </div>
            </div>
          </div>
        </div>
