...
/**
 * MVP — Gestão de Projetos (client-side)
 * - Botão "Carregar Resumo" para colar resumos de reuniões/emails
 * - Integra automaticamente timeline, responsáveis, próximos passos e tags
...
const STORAGE_KEY = "projetos_mvp_next_v3";
...
const AMOSTRA = [
  {
    id: uid(),
    titulo: "Nova Temporada SIC 2025",
    categoria: "PROJETOS PROMOS",
    estado: "Em Curso",
    responsaveis: ["Miguel Barriga", "Joana"],
    proximosPassos: ["Fechar packshot final", "Confirmar Bento para VT Abertura"],
    timeline: [
      { id: uid(), etiqueta: "Evento", data: "2025-09-07", status: "PREVISTA" },
      { id: uid(), etiqueta: "Cromas", data: "2025-08-18", status: "PREVISTA" }
    ],
    prioridade: "Alta",
    notas: "Cromas atrasados. Bento a confirmar.",
    tags: ["VT Abertura", "Cromas", "Evento"],
    historico: []
  },
  {
    id: uid(),
    titulo: "IEGCQT – Promo versão curta",
    categoria: "PROJETOS PROMOS",
    estado: "Em Curso",
    responsaveis: ["Miguel Barriga", "Franck", "Joana"],
    proximosPassos: ["Produzir spots de áudio", "Integrar packshot"],
    timeline: [{ id: uid(), etiqueta: "Entrega TV", data: "2025-08-17", status: "PREVISTA" }],
    prioridade: "Média",
    notas: "Música enviada; áudio com Franck e Joana.",
    tags: ["Rádio", "Podcasts", "Packshot"],
    historico: []
  },
  {
    id: uid(),
    titulo: "Vitória – Promo Disney",
    categoria: "PROJETOS PROMOS",
    estado: "Em Curso",
    responsaveis: ["Miguel Barriga", "Duarte"],
    proximosPassos: ["Gravar rádio com Cláudia", "Fechar aprovação Disney"],
    timeline: [
      { id: uid(), etiqueta: "Entrega", data: "2025-08-22", status: "PREVISTA" },
      { id: uid(), etiqueta: "Entrega", data: "2025-08-25", status: "CONFIRMADA" }
    ],
    prioridade: "Alta",
    notas: "Janela 21 a 25 agosto.",
    tags: ["Promo", "Disney", "Aprovação"],
    historico: []
  },
  {
    id: uid(),
    titulo: "Videocast Francisco Froes",
    categoria: "PROJETOS PUB",
    estado: "Em Curso",
    responsaveis: ["Miguel Barriga"],
    proximosPassos: ["Decidir sobre Jardim da Estrela"],
    timeline: [{ id: uid(), etiqueta: "Confirmação espaço", data: "2025-08-22", status: "PREVISTA" }],
    prioridade: "Média",
    notas: "Vitrines impedem gravação; aguardar Vanessa após 22 agosto.",
    tags: ["Location", "Jardim da Estrela"],
    historico: []
  }
];

// ===== Helpers de parsing para integrar automaticamente a partir de texto =====
const KW_PASSOS = [
  "gravar",
  "entregar",
  "enviar",
...
function extrairDatas(texto) {
  ...
  texto.replace(RE_DMY, (m) => {
    const [d, mo, y] = m.split(/[\/-]/);
    const dd = (d + "").padStart(2, "0");
    const mm = (mo + "").padStart(2, "0");
    push(`${y}-${mm}-${dd}`, ctx(texto, m));
    return m;
  });
  return out;
}
function ctx(txt, m) {
  ...
  const i = s.indexOf(m.toLowerCase());
...
function extrairTags(t) {
  ...
  const set = new Set();
  let m;
  while ((m = r.exec(t))) set.add(m[1]);
  return [...set];
}
function extrairResp(t) {
  const r = /@([\p{L}_.-]+(?:\s+[\p{L}_.-]+)*)/gu;
  const set = new Set();
  let m;
  while ((m = r.exec(t))) set.add(m[1].trim());
  return [...set];
}
function extrairPassos(t) {
  const linhas = t
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  return [...new Set(linhas.filter((l) => KW_PASSOS.some((k) => l.toLowerCase().startsWith(k) || l.toLowerCase().includes(` ${k} `))))].slice(0, 12);
}
function resumoCurto(t) {
  const ls = t.split(/\n/).map((s) => s.trim()).filter(Boolean);
  const fortes = ls.filter((l) => /(entrega|prazo|confirm|aprova|grava|bloque|pendente|datas?)/i.test(l));
  return (fortes.length ? fortes : ls).slice(0, 5).join("\n");
}
function parseTexto(t) {
  return {
    resumo: resumoCurto(t),
    proximosPassos: extrairPassos(t),
    timeline: extrairDatas(t),
    responsaveis: extrairResp(t),
    tags: extrairTags(t)
  };
}
...
export default function Page() {
  ...
  // NOVO: Modal de Carregar Resumo (reunião/email/nota)
  const [showResumo, setShowResumo] = useState(false);
  const [resumoProjetoId, setResumoProjetoId] = useState("");
  const [resumoTipo, setResumoTipo] = useState("reunião");
  const [resumoData, setResumoData] = useState(hojeISO());
  const [resumoTexto, setResumoTexto] = useState("");
  const [autoIntegrar, setAutoIntegrar] = useState(true);
  ...
  const filtrados = useMemo(() => {
    ...
    return projetos.filter((p) => {
      const texto = [p.titulo, p.notas, ...(p.tags || []), ...(p.responsaveis || [])]
        .join(" ")
        .toLowerCase();
  ...
  // ===== Carregar Resumo =====
  function abrirCarregarResumo(p = null) {
    setResumoProjetoId(p?.id || "");
    setResumoTipo("reunião");
    setResumoData(hojeISO());
    setResumoTexto("");
    setAutoIntegrar(true);
    setShowResumo(true);
  }
  function guardarResumo() {
    const alvo = resumoProjetoId || (projetos[0]?.id || "");
    if (!alvo) {
      alert("Escolhe um projeto");
      return;
    }
    const parsed = autoIntegrar ? parseTexto(resumoTexto) : null;
    ...
    setProjetos((prev) =>
      prev.map((p) => {
        ...
        novo.historico = [
          ...
          {
            id: uid(),
            data: new Date(resumoData).toISOString(),
            origem: resumoTipo,
            resumo: parsed?.resumo || resumoCurto(resumoTexto) || "",
        ...
        if (parsed) {
          ...
          const carimbo = `\n\n[${new Date().toLocaleString()}] Resumo (${resumoTipo}):\n${parsed.resumo}`;
  ...
  function mdProjeto(p) {
    ...
    const linhas = (p.timeline || [])
      .slice()
      .sort((a, b) => (a.data || "").localeCompare(b.data || ""))
      .map((t) => `- [${t.status}] ${t.etiqueta || "Sem etiqueta"} em ${fmt(t.data)}`)
      .join("\n");
    const ult = (p.historico || [])
      .slice(-3)
      .reverse()
      .map((h) => `- ${new Date(h.data).toLocaleString()} • ${h.origem} • ${h.resumo || "(sem resumo)"}`)
      .join("\n");
    const riscos = [];
...
${ult || "- n/d"}`;
  ...
  function mdGeral() {
    ...
    const proximas = todos
      .flatMap((p) => (p.timeline || []).map((t) => ({ p, t })))
      .filter(({ t }) => ["PREVISTA", "CONFIRMADA"].includes(t.status) && t.data && t.data >= hoje)
      .sort((a, b) => a.t.data.localeCompare(b.t.data))
      .slice(0, 18);
  ...
  return (
    <div className="container">
      <div className="header">
        ...
        <div className="flex">
          <button className="btn" onClick={() => setShowGeneral(true)}>
            Ponto de situação
          </button>
      ...
      <div className="tabs">
        {CATEGORIAS.map((c) => (
          <button key={c} className={"tab " + (c === cat ? "active" : "")} onClick={() => setCat(c)}>
            {c}
          </button>
        ))}
      </div>
      ...
      <div className="row">
        ...
        <label className="flex">
          <input type="checkbox" checked={alta} onChange={(e) => setAlta(e.target.checked)} /> Só prioridade alta
        </label>
      </div>
        ...
        {ESTADOS.map((est) => (
          <div className="col" key={est}>
            ...
            <div className="list">
              ...
              {filtrados
                .filter((p) => p.categoria === cat && p.estado === est)
                .map((p) => {
                  const divs = detectarDivergencias(p);
                  const atras = detectarAtrasos(p);
                    ...
                    <div className="card" key={p.id}>
                      <div className="flex" style={{ justifyContent: "space-between" }}>
                        ...
                        <div className="flex">
                          ...
                          <button className="btn" onClick={() => abrirCarregarResumo(p)}>
                            Carregar Resumo
                          </button>
                          <button
                            className="btn"
                          ...
                          <button className="btn" onClick={() => apagar(p.id)}>
                            Apagar
                          </button>
                        </div>
                      </div>
                      ...
                      {(p.proximosPassos || []).length > 0 && (
                        <div className="muted">
                          <strong>Próximos passos:</strong> {p.proximosPassos.join(" · ")}
                        </div>
                      )}
                        ...
                        <div className="timeline">
                          ...
                          {ult3.map((h) => (
                            <div key={h.id} className="flex">
                              <span className="badge">{h.origem}</span>
      ...
      {/* Modal: Carregar Resumo */}
      {showResumo && (
        <div className="modalBg" onClick={(e) => { if (e.target === e.currentTarget) setShowResumo(false); }}>
          <div className="modal">
            <h3>Carregar Resumo (Reunião / Email / Nota)</h3>
            <div className="grid">
              <select className="select" value={resumoProjetoId} onChange={(e) => setResumoProjetoId(e.target.value)}>
              ...
              <div className="row2">
                <select className="select" value={resumoTipo} onChange={(e) => setResumoTipo(e.target.value)}>
                  <option value="reunião">Reunião</option>
                  <option value="email">Email</option>
                  <option value="nota">Nota</option>
                </select>
                <input className="input" type="date" value={resumoData} onChange={(e) => setResumoData(e.target.value)} />
              </div>
              <textarea className="textarea" rows={10} placeholder="Cola aqui o resumo ou o texto integral do email/ata" value={resumoTexto} onChange={(e) => setResumoTexto(e.target.value)} />
              <label className="flex">
                <input type="checkbox" checked={autoIntegrar} onChange={(e) => setAutoIntegrar(e.target.checked)} /> Integrar automaticamente em timeline, responsáveis, próximos passos e tags
              </label>
              <div className="flex" style={{ justifyContent: "flex-end" }}>
                <button className="btn" onClick={() => setShowResumo(false)}>Cancelar</button>
                <button className="btn primary" onClick={guardarResumo}>Guardar</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="footer">Dados guardados localmente no navegador. Exporta o Markdown antes das reuniões.</div>
    </div>
  );
}
